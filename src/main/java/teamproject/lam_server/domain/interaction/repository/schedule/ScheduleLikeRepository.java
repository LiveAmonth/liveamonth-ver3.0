...
public interface ScheduleLikeRepository extends JpaRepository<ScheduleLike, Long> {
    @Modifying
    @Transactional
    @Query(value = "insert into schedule_like (from_member_id, to_schedule_id) values(:#{#request.from}, :#{#request.to})", nativeQuery = true)
    void like(@Param("request") InteractionRequest request);

    @Modifying
    @Transactional
    @Query(value = "delete from schedule_like where from_member_id = :#{#request.from} and to_schedule_id = :#{#request.to};", nativeQuery = true)
