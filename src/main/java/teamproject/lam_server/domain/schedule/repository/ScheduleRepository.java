...
public interface ScheduleRepository extends JpaRepository<Schedule, Long> {
    ...
    @Transactional
    @Query(value = "" +
            "insert into schedule (created_date, last_modified_date, member_id, title, city_name, start_date, end_date, public_flag) " +
            "values(now(), now(), :memberId, :#{#request.title}, :#{#request.city}, :#{#request.period.startDate}, :#{#request.period.endDate}, :#{#request.publicFlag})"
            , nativeQuery = true)
